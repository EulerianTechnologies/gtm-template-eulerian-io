...
The following parameters are always provided to every call sent to us, as long as they exist in the original dataLayer :
- **user_id** mapped to **uid**
For each call we auto-copy all additional parameters of the event, prefixed by **gtm-**, for non-standard keys (e.g. **currency** is sent as **gtm-currency**). Note that `tax`, `shipping`, `coupon`, and `currency` are only pulled in this way for the `purchase` and `generate_lead` events — they are not sent for other event types.
 
#### page_view
 
Standard call done.
 
**If** you have `page_view` events that are linked to another `event_name` you can provide a list through the **Similar PageView - Event Names** setting (`similar_page_view_event`).
This configuration entry allows you to list **all** events that **must be** considered as `page_view` events.
For example: `virtual_page_view,consented_trafic,hit_ok` will all be considered as `page_view` events.

You can also used the **Advanced event name remapping** for better control.
 
#### gtm.js
 
 In case you do not have a **page_view** event, you can check the box in the **Configuration** section ("Consider gtm.js event as page_view") allowing a **gtm.js** event name to be mapped to a page_view event, that way you are still able to trigger calls to Eulerian Marketing Platform.
...
**If** one of your custom events should actually be treated as one of the six standard events above (`page_view`, `view_item`, `add_to_cart`, `remove_from_cart`, `purchase`, `generate_lead`), you don't need to change your dataLayer — use the **Advanced event name remapping** table instead.
 
For each row you configure:
- **Match type**: `Exact match`, `Starts with`, `Contains`, or `Ends with`
- **Event name value**: the text to compare against the incoming event name (comparison is case-insensitive)
- **Treat as**: which of the six standard events the match should be handled as
Rules are evaluated **top-down** and the **first matching row wins**. Regardless of remapping, the event's original, raw name (as it appeared in the dataLayer) is always preserved and sent to Eulerian as the **ga-event_name** parameter — so downstream reporting still shows you which real event triggered the call.
 
Example: if your dataLayer fires `product_detail_viewed` instead of `view_item`, add a row with match type `Exact match`, value `product_detail_viewed`, treat as `view_item`.
